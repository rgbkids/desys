  ...
  if (provider === 'gemini') {
    ...
    try {
      ...
      if (msg.includes('429') || msg.includes('RESOURCE_EXHAUSTED')) {
        ...
        const res = await openai.chat.completions.create({
          model: DEFAULT_OPENAI_MINI_MODEL, temperature: 0.4,
          messages: [
            { role: 'system', content: SYSTEM },
            { role: 'system', content: `現在のトークン: ${JSON.stringify(tokens)}` },
            { role: 'user', content: `コンポーネント名: ${compName}\n要件: ${prompt}\n出力はコードのみ。TSX。` }
    ...
    if (provider === 'claude') {
      ...
      try {
        ...
        const res = await openai.chat.completions.create({ model: DEFAULT_OPENAI_MINI_MODEL, temperature: 0.4, messages })
        code = res.choices[0]?.message?.content?.trim() || ''
      }
    } else {
      const key = process.env.OPENAI_API_KEY
